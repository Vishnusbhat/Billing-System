...
const NewInvoice = () => {
  ...
  const handleCustomerChange = async (newValue) => {
    ...
    if (newValue && newValue.__isNew__) {
      // New customer, clear the phone and email fields
      setCustomerPhone('');
      setCustomerEmail('');
    } else if (newValue) {
      // Existing customer, fetch their details
      try {
      ...
      // No customer selected, clear the phone and email fields
      setCustomerPhone('');
      setCustomerEmail('');
    }
  };
  ...
  const handleAddItem = (item) => {
    ...
    const updatedTotalCost = selectedItems.reduce((total, selectedItem) => (selectedItem.price * selectedItem.quantity), 0);
    ...
    <div className="new-invoice-container">
      ...
      <div className="search-bar">
        <input
          ...
          placeholder="Search..."
        />
        <button className="invoice-button" onClick={handleSubmit}>Invoice</button>
      </div>
